...
- [x] Rust后端核心框架
- [x] Vue3前端基础框架
- [x] 微信信息获取（完善，支持从WeChatWin.dll读取版本）
- [x] 数据库解密（完善，带验证）
- [x] Git配置和自动提交脚本
- [x] 版本偏移量配置导入
- [x] 所有数据库模块（MSG、Contact、Media、Favorite、Sns、Merge）
- [x] 所有主要API接口（聊天、统计、媒体、导出、合并、收藏、朋友圈）
- [x] 消息内容解析（XML、LZ4、BytesExtra）
- [x] 数据库合并功能
- [x] 内存映射支持（WeChatWin.dll基址获取）
- [x] 输入验证工具（数据库路径、密钥、时间范围、分页）
- [x] 前端所有页面（首页、聊天记录、联系人、收藏、朋友圈、统计分析、数据导出、清理、工具、设置）
- [x] 消息类型组件（7种类型）
- [x] API集成（所有前端页面与后端API对接）
- [x] 代码重构（所有文件控制在300行以内，单一职责）
